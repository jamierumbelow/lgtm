...
export function renderMarkdown(analysis: Analysis): string {
  ...
  // Review Guidance
  if (analysis.reviewGuidance) {
    lines.push("## Review Guidance");
    lines.push("");
    lines.push(`> ${analysis.reviewGuidance.replace(/\n/g, "\n> ")}`);
    lines.push("");
  }
  ...
  for (const group of analysis.changeGroups) {
    ...
    if (group.verdict) {
      lines.push(`> **Verdict:** ${group.verdict}`);
      lines.push("");
    }
    ...
    if (group.suggestions && group.suggestions.length > 0) {
      ...
      for (const suggestion of group.suggestions) {
        const prefix = `[${suggestion.severity.toUpperCase()}]`;
        const fileRef = suggestion.file ? ` (\`${suggestion.file}\`)` : "";
        lines.push(`- ${prefix} ${suggestion.text}${fileRef}`);
      }
      lines.push("");
    }
  ...
  lines.push("---");
  lines.push(
    '*Generated by [lgtm](https://github.com/jamierumbelow/lgtm) - because "lgtm" should mean something*'
  );
