import { aggregateContributors } from "../github/blame.js";
...
export function getAnalysisShape(options = {}) {
    return {
...
export function getMissingAnalysisParts(analysis, shape) {
    const existingQuestions = analysis?.questions ?? [];
    const existingQuestionIds = new Set(existingQuestions.map((question) => question.id));
    const missingQuestionIds = shape.requiredQuestionIds.filter((id) => !existingQuestionIds.has(id));
    ...
    const needsChangesetQuestions = !analysis?.changeGroups ||
        analysis.changeGroups.some((group) => !group.reviewQuestions || group.reviewQuestions.length === 0);
    const needsTraces = shape.requireTraces && (!analysis?.traces || analysis.traces.length === 0);
    return {
        needsChangeGroups,
        needsContributors,
...
export async function analyzeChanges(prData, options) {
    ...
    const buildPartialAnalysis = (groups) => ({
        prUrl: prData.url,
        title: prData.title,
    ...
    if (options.useLLM) {
        if (options.verbose) {
            console.log(`[lgtm] answering changeset questions for ${changeGroupsWithQuestions.length} change groups`);
        }
        const answeredChangesets = await answerChangesetQuestionsWithLLM(changeGroupsWithQuestions, {
...
export async function ensureAnalysis(prData, options, existing) {
    if (!existing) {
    ...
    const changesetQuestionsResult = buildChangesetQuestions(changeGroups, existing.changeGroups);
    let changeGroupsWithQuestions = changesetQuestionsResult.changeGroups;
    ...
    const buildPartialAnalysis = (groups, questions = existing.questions) => ({
        ...existing,
    ...
    const questionsResult = buildQuestions(prData, changeGroupsWithQuestions, contributors, existing.questions);
    const updatedAnalysis = buildPartialAnalysis(changeGroupsWithQuestions, questionsResult.questions);
    const updated = missing.needsChangeGroups ||
    ...
    return {
        ...
        missing: {
            ...missing,
            missingQuestionIds: shape.requiredQuestionIds.filter((id) => !questionsResult.questionIds.has(id)),
        },
    };
}
function calculateStats(prData) {
...
async function buildChangeGroups(prData, options) {
    return chunkDiff(prData.diff, prData.files, { useLLM: options.useLLM });
}
async function buildContributors(prData) {
    const filePaths = prData.files.map((f) => f.path);
    return aggregateContributors(filePaths);
}
function buildSuggestedReviewers(prData, contributors) {
    return contributors
        .filter((c) => !prData.author || !c.email.includes(prData.author))
        .slice(0, 3)
        .map((c) => c.name);
}
function buildQuestions(prData, changeGroups, contributors, existingQuestions = []) {
    ...
    const standardQuestions = OVERVIEW_QUESTIONS.map((question) => {
        ...
        const context = getQuestionContext(question.id, prData, changeGroups, contributors);
        if (!existing) {
            updated = true;
            return {
                ...question,
        ...
        return {
            ...
            context: existing.context && existing.context.length > 0
                ? existing.context
                : context,
        };
    });
    const standardIds = new Set(OVERVIEW_QUESTIONS.map((question) => question.id));
    const extraQuestions = existingQuestions
        .filter((question) => !standardIds.has(question.id) && question.category !== "changeset")
        .map((question) => {
        const normalized = {
            ...question,
...
function buildChangesetQuestions(changeGroups, existingChangeGroups = []) {
    const existingById = new Map(existingChangeGroups.map((group) => [group.id, group]));
    let updated = false;
...
function getQuestionContext(questionId, prData, changeGroups, contributors) {
    switch (questionId) {
        case "reviewers":
            return contributors
                .slice(0, 5)
                ...
                .join("\n");
        case "new-symbols":
            return (changeGroups.flatMap((g) => g.symbolsIntroduced || []).join(", ") ||
                "Analysis pending...");
        case "decomposition":
