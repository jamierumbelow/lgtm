 ...
 */
export async function chunkDiff(diff, files, options = {}) {
 ...
 */
export function chunkDiffHeuristic(diff) {
...
function inferGroupTitle(dir, files) {
...
function inferChangeType(files) {
    ...
    if (paths.every((p) => p.includes("test") || p.includes("spec")))
        return "test";
    if (paths.every((p) => p.includes("readme") || p.includes("doc") || p.endsWith(".md")))
        return "docs";
    if (paths.every((p) => p.includes("config") ||
        p.endsWith(".json") ||
        p.endsWith(".yaml") ||
        p.endsWith(".yml") ||
        p.endsWith(".toml")))
...
function extractNewSymbols(files) {
    const symbols = [];
...
function extractModifiedSymbols(files) {
    const symbols = [];
    for (const file of files) {
        for (const hunk of file.hunks) {
