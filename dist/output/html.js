    ...
    <div class="overview-container">
      ...
      <main class="main">
        ...
        : ""}
          ...
          ${analysis.prUrl
        ? ` · <a href="${escapeHtml(analysis.prUrl)}" style="color: var(--accent)">View on GitHub</a>`
        : ""}
        </p>
...
function renderSummarySlide(analysis) {
      ...
      <div class="summary-slide">
        <div class="summary-content">
          <div class="summary-header">
            <h1 class="summary-title">${escapeHtml(analysis.title ||
        `${analysis.baseBranch} ← ${analysis.headBranch}`)}</h1>
...
function renderReviewSlides(analysis) {
    return analysis.changeGroups
        .map((group, index) => renderSlide(group, index + 1, analysis) // +1 because summary is at index 0
    )
        .join("\n");
}
function renderSlide(group, index, analysis) {
      ...
      <div class="meta-panel">
        ...
        ? `
          <div class="meta-section">
            ...
            : q.context
                ? `<pre style="white-space: pre-wrap; font-size: 12px;">${escapeHtml(q.context)}</pre>`
...
function renderIntegratedDiff(hunks) {
    let html = "";
...
function renderSideBySideDiff(hunks) {
    let html = "";
    for (const { hunk } of hunks) {
        html += `
        <div class="diff-hunk-header">@@ -${hunk.oldStart},${hunk.oldLines} +${hunk.newStart},${hunk.newLines} @@ ${escapeHtml(hunk.header)}</div>
      `;
        const lines = hunk.content.split("\n");
        let oldLine = hunk.oldStart;
        let newLine = hunk.newStart;
        for (const line of lines) {
          ...
          <div class="diff-row ${lineClass}">
            ...
            <span class="line-content old">${oldMarker}${escapeHtml(oldContent)}</span>
            <span class="line-number new">${newLineNum}</span>
            <span class="line-content new">${newMarker}${escapeHtml(newContent)}</span>
          </div>
        `;
        }
    }
...
function getFileStatus(file, hunks) {
...
function getRelevantQuestions(group, analysis) {
    ...
    if (isStructural) {
        ...
        return questions
            .filter((q) => ["abstractions", "duplication", "compatibility"].includes(q.id))
            .slice(0, 3);
    }
...
function renderGenerationMeta(analysis) {
    const hasMetadata = analysis.generationTimeMs || analysis.tokenCount || analysis.costUsd;
    if (!hasMetadata)
        return "";
    const formatTime = (ms) => {
        if (ms < 1000)
    ...
    <div class="generation-meta">
      ${analysis.generationTimeMs
        ...
        <div class="generation-meta-item">
          <span>⏱</span>
          <span class="generation-meta-value">${formatTime(analysis.generationTimeMs)}</span>
        </div>
      `
        : ""}
      ${analysis.tokenCount
        ...
        <div class="generation-meta-item">
          <span>◈</span>
          <span class="generation-meta-value">${formatTokens(analysis.tokenCount)}</span>
          <span>tokens</span>
        </div>
      `
        : ""}
      ${analysis.costUsd
        ...
        <div class="generation-meta-item">
          <span>$</span>
          <span class="generation-meta-value">${formatCost(analysis.costUsd)}</span>
        </div>
      `
        : ""}
    </div>
