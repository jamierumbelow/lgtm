...
import { DEFAULT_MODEL, DEFAULT_CHANGESET_QUESTION_CONCURRENCY, } from "../config.js";
...
const BATCHED_QUESTION_IDS = Object.keys(BatchedAnswersSchema.shape);
export async function answerChangesetQuestionsWithLLM(changeGroups, options = {}) {
    let updated = false;
    const log = options.log ??
        (options.verbose ? (message) => console.log(message) : undefined);
    const maxConcurrent = Math.max(1, Number(options.maxConcurrent ?? DEFAULT_CHANGESET_QUESTION_CONCURRENCY));
    const tasks = buildChangesetTasks(changeGroups, log);
    if (tasks.length === 0) {
        return { changeGroups, updated };
    }
    log?.(`[lgtm] processing ${tasks.length} changesets (parallel=${maxConcurrent})`);
    ...
    await runWithConcurrency(tasks, maxConcurrent, async ({ group }) => {
        ...
        try {
            ...
            log?.(`[lgtm] answering all questions for "${group.title}" in single call`);
            const systemPrompt = loadPrompt("review-questions/batched.v1.txt", effectiveModel);
            const userPrompt = buildUserPrompt(group);
            const answers = await generateStructured(systemPrompt, userPrompt, BatchedAnswersSchema, {
                temperature: 0.2,
                maxTokens: 2048,
                ...options,
            ...
            console.error(`[lgtm] failed answering questions for "${group.title}"`, `model=${effectiveModel}`, message);
            throw error;
        }
    });
    return { changeGroups, updated };
}
function buildChangesetTasks(changeGroups, log) {
    const tasks = [];
    for (const group of changeGroups) {
        ...
        const unansweredQuestions = group.reviewQuestions.filter((q) => q.category === "changeset" &&
            BATCHED_QUESTION_IDS.includes(q.id) &&
            (!q.answer || q.answer.trim().length === 0));
        if (unansweredQuestions.length === 0) {
            log?.(`[lgtm] skipping changeset "${group.title}" (all questions answered)`);
            continue;
        }
        log?.(`[lgtm] changeset "${group.title}" (${unansweredQuestions.length} unanswered questions)`);
        tasks.push({ group });
    }
    return tasks;
}
function applyAnswersToGroup(group, answers) {
    if (!group.reviewQuestions)
        return;
...
function extractErrorExtras(error) {
    const anyError = error;
    const extras = {};
    if (anyError.statusCode)
        extras.statusCode = anyError.statusCode;
    if (anyError.url)
        extras.url = anyError.url;
    if (anyError.responseBody)
        extras.responseBody = anyError.responseBody;
    if (anyError.data)
        extras.data = anyError.data;
    if (anyError.response)
        extras.response = anyError.response;
    if (anyError.request)
        extras.request = anyError.request;
