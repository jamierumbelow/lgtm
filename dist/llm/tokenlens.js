...
function buildTokenArgs(record) {
    return {
...
function getModelId(tokenlens, model) {
    if (tokenlens.isModelId?.(model))
        return model;
...
function resolveRecordCost(tokenlens, record, log) {
    if (!record.model)
        return undefined;
    ...
    if (resolvedModel !== record.model) {
        log?.(`[tokenlens] Resolved model alias ${record.model} -> ${String(resolvedModel)}.`);
    }
...
export async function calculateTokenlensCost(records, log) {
    let tokenlens;
    try {
        const imported = await import("tokenlens");
        tokenlens = (imported.default ?? imported);
    }
    catch {
    ...
    for (const record of records) {
        log?.(`[tokenlens] Usage: model=${record.model ?? "unknown"} input=${record.inputTokens} output=${record.outputTokens} cacheWrite=${record.cacheWriteTokens} cacheRead=${record.cacheReadTokens} total=${record.totalTokens}.`);
        ...
        if (cost !== undefined) {
            ...
            const tokenCosts = record.model
                ...
                : undefined;
            log?.(`[tokenlens] Could not resolve cost for model=${String(resolvedModel)} tokens=${record.totalTokens}. Meta=${JSON.stringify(meta ?? null)} TokenCosts=${JSON.stringify(tokenCosts ?? null)}.`);
        }
    }
    if (!hasValue) {
        log?.(`[tokenlens] No cost calculated. Missing model: ${missingModel}, unresolved cost: ${missingCost}. Exports: ${moduleKeys.join(", ") || "(none)"}`);
        return undefined;
    }
